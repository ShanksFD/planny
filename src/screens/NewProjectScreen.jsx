...
function NewProjectScreen({ history }) {
    ...
    if (true) {
      dispatch(
        registerProject({
          ...
          manager_id:
            managerId === -1 ? projectManagerList.users[0].uid : managerId,
          files: data.files,
        })
      );
    }
  ...
  const submitFiles = (data, event) => {
    if (data.files.length === 0) setMessage("Please upload a valid files");
    else {
    ...
    <FormContainer>
      ...
      <Form onSubmit={handleSubmit(submitFiles)}>
        ...
        <Button
          ...
          variant="primary"
          disabled={
            projectManagerList.error || projectManagerList.users.length === 0
          }
        >
          SAVE
        </Button>
